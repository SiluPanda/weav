...
impl WeavClient {
    ...
    async fn get_graph(&self, name: &str) -> (StatusCode, Value) {
        let resp = self
            .client
            .get(format!("{}/v1/graphs/{}", self.base_url, name))
            .send()
            .await
            .unwrap();
        let status = resp.status();
    ...
    async fn delete_graph(&self, name: &str) -> (StatusCode, Value) {
        let resp = self
            .client
            .delete(format!("{}/v1/graphs/{}", self.base_url, name))
            .send()
            .await
            .unwrap();
        let status = resp.status();
    ...
    async fn add_node(&self, graph: &str, req: &AddNodeRequest) -> (StatusCode, Value) {
        let resp = self
            .client
            .post(format!("{}/v1/graphs/{}/nodes", self.base_url, graph))
            .json(req)
            .send()
            .await
            .unwrap();
    ...
    async fn bulk_add_nodes(&self, graph: &str, nodes: &[AddNodeRequest]) -> (StatusCode, Value) {
        let resp = self
            .client
            .post(format!("{}/v1/graphs/{}/nodes/bulk", self.base_url, graph))
    ...
    async fn add_edge(&self, graph: &str, req: &AddEdgeRequest) -> (StatusCode, Value) {
        let resp = self
            .client
            .post(format!("{}/v1/graphs/{}/edges", self.base_url, graph))
            .json(req)
            .send()
            .await
            .unwrap();
    ...
    async fn bulk_add_edges(&self, graph: &str, edges: &[AddEdgeRequest]) -> (StatusCode, Value) {
        let resp = self
            .client
            .post(format!("{}/v1/graphs/{}/edges/bulk", self.base_url, graph))
    ...
    async fn context(&self, body: &Value) -> (StatusCode, Value) {
        let resp = self
            .client
            .post(format!("{}/v1/context", self.base_url))
            .json(body)
            .send()
            .await
            .unwrap();
    ...
    async fn snapshot(&self) -> (StatusCode, Value) {
        let resp = self
            .client
            .post(format!("{}/v1/snapshot", self.base_url))
            .send()
            .await
            .unwrap();
        let status = resp.status();
    ...
    async fn event_stream(&self, path: &str) -> reqwest::Response {
        self.client
            .get(format!("{}{}", self.base_url, path))
            .send()
            .await
...
fn extract_node_id(body: &Value) -> u64 {
    body["data"]["node_id"].as_u64().unwrap()
}
fn extract_edge_id(body: &Value) -> u64 {
    body["data"]["edge_id"].as_u64().unwrap()
}

async fn read_sse_events(response: &mut reqwest::Response, expected: usize) -> Vec<Value> {
...
async fn setup(graph_name: &str) -> (TestServer, WeavClient, String) {
    ...
    assert_eq!(
        status,
        StatusCode::CREATED,
        "failed to create graph {graph_name}"
    );
...
async fn create_two_nodes(client: &WeavClient, graph: &str) -> (u64, u64) {
    let (_, b1) = client
        .add_node(graph, &make_node_with_props("a", json!({"name": "node-a"})))
        .await;
    let (_, b2) = client
        .add_node(graph, &make_node_with_props("b", json!({"name": "node-b"})))
        .await;
...
async fn build_knowledge_graph(client: &WeavClient, graph: &str) -> (u64, u64, u64) {
    let alice = make_node_with_key(
        "person",
        "alice",
        json!({"name": "Alice", "bio": "Alice is a software engineer who loves Rust."}),
    );
    let bob = make_node_with_key(
        "person",
        "bob",
        json!({"name": "Bob", "bio": "Bob is a data scientist specializing in ML."}),
    );
    let acme = make_node_with_key(
        "company",
        "acme",
        json!({"name": "Acme Corp", "description": "Acme is a tech company."}),
    );
...
async fn test_info_endpoint() {
    ...
    assert!(
        info_text.contains("weav-server"),
        "info should contain 'weav-server', got: {info_text}"
    );
}
...
#[tokio::test]
async fn test_graph_events_sse_replays_and_streams_live() {
    let (_server, client, graph) = setup("events_sse").await;

    let (_status, first_body) = client
        .add_node(
            &graph,
            &make_node_with_props("person", json!({ "name": "Alice" })),
        )
        .await;
    assert_eq!(extract_node_id(&first_body), 1);

    let mut response = client
        .event_stream(&format!(
            "/v1/graphs/{graph}/events?since_sequence=0&replay_limit=10"
        ))
        .await;
    assert_eq!(response.status(), StatusCode::OK);

    let replayed = read_sse_events(&mut response, 2).await;
    assert_eq!(replayed[0]["kind"], "graph_created");
    assert_eq!(replayed[0]["graph"], graph);
    assert_eq!(replayed[1]["kind"], "node_created");
    let replay_payload: Value =
        serde_json::from_str(replayed[1]["payload_json"].as_str().unwrap()).unwrap();
    assert_eq!(replay_payload["node_id"], 1);

    let (_status, second_body) = client
        .add_node(
            &graph,
            &make_node_with_props("person", json!({ "name": "Bob" })),
        )
        .await;
    assert_eq!(extract_node_id(&second_body), 2);

    let live = read_sse_events(&mut response, 1).await;
    assert_eq!(live[0]["kind"], "node_created");
    let live_payload: Value =
        serde_json::from_str(live[0]["payload_json"].as_str().unwrap()).unwrap();
    assert_eq!(live_payload["node_id"], 2);
}

#[tokio::test]
async fn test_graph_events_sse_replay_from_sequence() {
    let (_server, client, graph) = setup("events_seq").await;
    ...
    let mut initial = client
        ...
        .await;
    let replayed = read_sse_events(&mut initial, 3).await;
    let cutoff = replayed[1]["sequence"].as_u64().unwrap();
    drop(initial);

    let mut resumed = client
        .event_stream(&format!(
            "/v1/graphs/{graph}/events?since_sequence={cutoff}&replay_limit=10"
        ))
        .await;
    let resumed_events = read_sse_events(&mut resumed, 1).await;
    assert_eq!(resumed_events[0]["kind"], "node_created");
    let resumed_payload: Value =
        serde_json::from_str(resumed_events[0]["payload_json"].as_str().unwrap()).unwrap();
    assert_eq!(resumed_payload["node_id"], 2);
}
...
async fn test_update_node_properties() {
    ...
    let (status, _) = client
        .update_node(
            &graph,
            id,
            &json!({"properties": {"name": "Bob", "city": "NYC"}}),
        )
        .await;
    assert_eq!(status, StatusCode::OK);
...
async fn test_update_node_preserves_unmodified() {
    ...
    let id = extract_node_id(&body);
    client
        .update_node(&graph, id, &json!({"properties": {"name": "Bob"}}))
        .await;
    let (_, body) = client.get_node(&graph, id).await;
    assert_eq!(body["data"]["properties"]["name"], "Bob");
...
async fn test_update_node_not_found() {
    let (_s, client, graph) = setup("g").await;
    let (status, _) = client
        .update_node(&graph, 99999, &json!({"properties": {"name": "x"}}))
        .await;
    assert_eq!(status, StatusCode::NOT_FOUND);
}
...
async fn test_bulk_add_nodes() {
    let (_s, client, graph) = setup("g").await;
    let nodes: Vec<AddNodeRequest> = (0..10)
        .map(|i| make_node_with_props("person", json!({"name": format!("p-{}", i)})))
        .collect();
...
async fn test_bulk_add_nodes_retrievable() {
    let (_s, client, graph) = setup("g").await;
    let nodes: Vec<AddNodeRequest> = (0..5)
        .map(|i| {
            make_node_with_key(
                "person",
                &format!("key-{i}"),
    ...
    let ids: Vec<u64> = body["data"]["node_ids"]
        .as_array()
        .unwrap()
        .iter()
...
async fn test_bulk_add_edges() {
    ...
    let (_, body) = client.bulk_add_nodes(&graph, &nodes).await;
    let nids: Vec<u64> = body["data"]["node_ids"]
        .as_array()
        .unwrap()
        .iter()
        .map(|v| v.as_u64().unwrap())
        .collect();
    let edges: Vec<AddEdgeRequest> = (0..5)
        .map(|i| AddEdgeRequest {
            source: nids[i],
            target: nids[i + 1],
...
async fn test_bulk_large_batch() {
    let (_s, client, graph) = setup("g").await;
    let nodes: Vec<AddNodeRequest> = (0..100)
        .map(|i| make_node_with_props("item", json!({"index": i})))
        .collect();
    let (_, body) = client.bulk_add_nodes(&graph, &nodes).await;
    let nids: Vec<u64> = body["data"]["node_ids"]
        .as_array()
    ...
    let edges: Vec<AddEdgeRequest> = (0..99)
        .map(|i| AddEdgeRequest {
            source: nids[i],
            target: nids[i + 1],
...
async fn test_add_edge_with_weight() {
    ...
    assert!(
        (weight - 0.75).abs() < 0.01,
        "weight should be 0.75, got {weight}"
    );
}
...
async fn test_add_edge_with_properties() {
    ...
    let edge = AddEdgeRequest {
        source: n1,
        target: n2,
        label: "interacted".to_string(),
        weight: None,
        properties: Some(json!({"type": "meeting"})),
    };
...
async fn test_add_edge_invalid_source() {
    ...
    assert!(
        ...
        "invalid source should fail, got {status}"
    );
    assert_eq!(body["success"], false);
}
...
async fn test_context_with_embedding() {
    let (_s, client, graph) = setup("g").await;
    let mut emb_a: Vec<f32> = vec![0.0; 1536];
    emb_a[0] = 1.0;
    let mut emb_b: Vec<f32> = vec![0.0; 1536];
    emb_b[0] = 0.9;
    emb_b[1] = 0.1;
    let node_a = AddNodeRequest {
        label: "doc".to_string(),
        properties: Some(json!({"content": "Rust programming"})),
        embedding: Some(emb_a.clone()),
        entity_key: Some("doc-rust".to_string()),
    };
    let node_b = AddNodeRequest {
        label: "doc".to_string(),
        properties: Some(json!({"content": "Python programming"})),
        embedding: Some(emb_b),
        entity_key: Some("doc-python".to_string()),
    };
    client.add_node(&graph, &node_a).await;
    client.add_node(&graph, &node_b).await;
    let (status, body) = client
        .context(&json!({"graph": graph, "embedding": emb_a, "budget": 4096}))
        .await;
    assert_eq!(status, StatusCode::OK);
    assert!(body["data"]["nodes_included"].as_u64().unwrap() > 0);
}
...
async fn test_context_with_both_seeds() {
    let (_s, client, graph) = setup("g").await;
    let mut emb: Vec<f32> = vec![0.0; 1536];
    emb[0] = 1.0;
    let node_a = AddNodeRequest {
        label: "doc".to_string(),
        properties: Some(json!({"content": "Vector search"})),
        embedding: Some(emb.clone()),
        entity_key: Some("doc-vec".to_string()),
    };
    ...
    let (status, body) = client
        .context(
            &json!({"graph": graph, "embedding": emb, "seed_nodes": ["alice"], "budget": 4096}),
        )
        .await;
    assert_eq!(status, StatusCode::OK);
    assert!(body["data"]["nodes_included"].as_u64().unwrap() > 0);
...
async fn test_context_with_budget() {
    ...
    let (status, body) = client
        .context(&json!({"graph": graph, "seed_nodes": ["art-0"], "budget": 50, "max_depth": 1}))
        .await;
    assert_eq!(status, StatusCode::OK);
...
async fn test_context_empty_graph() {
    let (_s, client, graph) = setup("g").await;
    let (status, body) = client
        .context(&json!({"graph": graph, "seed_nodes": ["nonexistent"], "budget": 4096}))
        .await;
    assert_eq!(status, StatusCode::OK);
...
async fn test_context_graph_not_found() {
    ...
    let client = WeavClient::new(&server);
    let (status, body) = client
        .context(&json!({"graph": "nonexistent", "seed_nodes": ["foo"], "budget": 4096}))
        .await;
    assert!(
        status.is_client_error() || status.is_server_error(),
        "context on missing graph should fail, got {status}"
    );
    assert_eq!(body["success"], false);
}
...
async fn test_missing_required_field() {
    ...
    assert!(
        status.is_client_error(),
        "missing 'name' field should return 4xx, got {status}"
    );
}
...
async fn test_special_chars_in_graph_name() {
    ...
    let (status, body) = client
        .add_node(&graph, &make_node_with_props("item", json!({"val": 1})))
        .await;
    assert_eq!(status, StatusCode::CREATED);
    let id = extract_node_id(&body);
    let (status, _) = client.get_node(&graph, id).await;
...
async fn test_node_properties_null_values() {
    ...
    let node = make_node_with_props(
        "thing",
        json!({"name": "test", "optional": null, "active": true}),
    );
    let (_, body) = client.add_node(&graph, &node).await;
    let id = extract_node_id(&body);
    let (status, body) = client.get_node(&graph, id).await;
...
async fn test_node_properties_arrays() {
    ...
    let node = make_node_with_props(
        "item",
        json!({"tags": ["rust", "database", "graph"], "name": "weav"}),
    );
    let (_, body) = client.add_node(&graph, &node).await;
    let id = extract_node_id(&body);
    let (_, body) = client.get_node(&graph, id).await;
...
async fn test_graph_full_lifecycle() {
    ...
    let (_, b1) = client
        .add_node(
            &graph,
            &make_node_with_props("person", json!({"name": "A"})),
        )
        .await;
    let (_, b2) = client
        .add_node(
            &graph,
            &make_node_with_props("person", json!({"name": "B"})),
        )
        .await;
...
async fn test_concurrent_node_creation() {
    ...
    engine
        .execute_command(
            weav_query::parser::Command::GraphCreate(weav_query::parser::GraphCreateCmd {
                name: "g".to_string(),
                config: None,
            }),
            None,
...
async fn test_concurrent_graph_operations() {
    ...
    for task_id in 0..4u32 {
        ...
        handles.push(tokio::spawn(async move {
            c.post(format!("{}/v1/graphs", url_base))
                .json(&json!({"name": g}))
                .send()
                .await
                .unwrap();
            for i in 0..20u32 {
                let resp = c
                    .post(format!("{}/v1/graphs/{}/nodes", url_base, g))
            ...
            let resp = c
                .get(format!("{}/v1/graphs/{}", url_base, g))
                .send()
                .await
                .unwrap();
...
async fn test_concurrent_read_write() {
    ...
    for i in 0..20u32 {
        engine
            .execute_command(
                weav_query::parser::Command::NodeAdd(weav_query::parser::NodeAddCmd {
                    graph: "g".to_string(),
...
async fn test_multiple_graphs_isolation() {
    ...
    for _ in 0..3 {
        client.add_node("g1", &make_node("a")).await;
    }
    for _ in 0..5 {
        client.add_node("g2", &make_node("b")).await;
    }
    for _ in 0..7 {
        client.add_node("g3", &make_node("c")).await;
    }
...
async fn test_e2e_empty_json_body() {
    ...
    let (status, _) = client.raw_post("/v1/graphs/empty_json_g/nodes", "{}").await;
    assert!(
        status.is_client_error(),
...
async fn test_e2e_self_loop_via_http() {
    ...
    let (_, body) = client
        .add_node(
            &graph,
            &make_node_with_props("item", json!({"name": "solo"})),
        )
        .await;
...
async fn test_e2e_context_with_large_max_depth() {
    ...
    let (_, a) = client
        .add_node(
            &graph,
            &make_node_with_key("item", "a", json!({"name": "A"})),
        )
        .await;
    let (_, b) = client
        .add_node(
            &graph,
            &make_node_with_key("item", "b", json!({"name": "B"})),
        )
        .await;
    let (_, c) = client
        .add_node(
            &graph,
            &make_node_with_key("item", "c", json!({"name": "C"})),
        )
        .await;
